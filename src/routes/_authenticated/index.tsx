...
function NoteAttachments({
  ...
  const attachmentsQ = useQuery({
    ...
    queryFn: () => listAttachments({ data: { note_id: noteId } }),
  });
  const attachments = (attachmentsQ.data ?? []) as Attachment[];
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        const path = `${uid}/${noteId}/${Date.now()}-${safe}`;
        ...
        if (file.type.startsWith("image/")) {
          const { url } = await signAttachment({ data: { storage_path: path } });
          onInsertMarkdown(`![${file.name}](${url})`);
        }
      }
      toast.success("Uploaded");
      qc.invalidateQueries({ queryKey: ["attachments", noteId] });
    } catch (e) {
  ...
  const openFile = async (a: Attachment) => {
    try {
      const { url } = await signAttachment({ data: { storage_path: a.storage_path } });
  ...
  const removeFile = async (a: Attachment) => {
    ...
    try {
      await deleteAttachment({ data: { id: a.id } });
      qc.invalidateQueries({ queryKey: ["attachments", noteId] });
    ...
    <div className="border-b bg-muted/10 px-3 py-2">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] text-muted-foreground">
          Images, PDFs, Word, anything · stored securely
        </span>
      </div>
